    ...
    def run(self):
        try:
            if mawin.test_flag=="DC":
                ...
                #self.TxBW_test()
                #add raw amplitude @1.5G judgement
                self.TxBW_test_1123()
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            if str(mawin.channel[i]) == '13':
                ...
                # left work here to store the data
                data_scan = mawin.dc_scapOPdata(st)
                print('CH13_scanOp state 4: YIminYQqua_SweepYP:\n', data_scan)
                # data_scan.to_csv(data_sw1)
        ...
        if mawin.test_type=='Normal':
            ...
            gf.write_eeprom_finalKT58(mawin,sn,test_result, mawin.device_type)
        else:
            print('非正常生产测试，不执行EEPROM写入...')
        # print('Print out page 7,8,9 to check')
        # time.sleep(0.1)
        # gf.read_eeprom_information(mawin,7)
        # time.sleep(0.1)
        # gf.read_eeprom_information(mawin,8)
        # time.sleep(0.1)
        # gf.read_eeprom_information(mawin,9)
        # time.sleep(0.1)
        self.sig_print.emit('测试完成!')
        self.sig_staColor.emit('green')
        self.sig_but.emit('开始')
        ...
        judge_ER_child = test_result.iloc[:, [14, 15, 17, 18]]
        judge_ER_mother = test_result.iloc[:, [16, 19]]
        # DC
        mawin.finalResult = 'Pass'
        if not spec_IL[0] < np.min(judge_IL) < spec_IL[1]:
            print('IL failed detected: ')
            mawin.finalResult='Fail'
        if abs(np.min(judge_TxPDL)) < abs(np.max(judge_TxPDL)):
            TxPDL_resu = np.max(judge_TxPDL)
        else:
            TxPDL_resu = np.min(judge_TxPDL)
        if not spec_Tx_PDL[0] < np.min(TxPDL_resu) < spec_Tx_PDL[1]:
            print('PDL failed detected: ')
            mawin.finalResult = 'Fail'
        if not spec_ER_child[0] < np.min(np.min(judge_ER_child)) < spec_ER_child[1]:
            print('ER child failed detected: ')
            mawin.finalResult='Fail'
        if not spec_ER_mother[0] < np.min(np.min(judge_ER_mother)) < spec_ER_mother[1]:
            print('ER mother failed detected: ')
            mawin.finalResult='Fail'
        ...
        wb.save(report_judge)
        wb.close()
        # # record time and print out status,then close save log file
        # mawin.end = time.time()
        # utime = str(time.strftime("%H:%M:%S", time.gmtime(mawin.end - mawin.start)))
        # self.sig_print.emit('测试完成，用时：' + utime)
        # print('测试完成，用时：' + utime)
        # try:
        #     sys.stdout.CloseLogFile()
        #     sys.stdout.log_enabled = False
        ...
        wb.sheets(2).activate()
        # info=gf.read_eeprom_information(mawin,1)
        # if info[15] ==0 or info[16] ==0 or info[17] ==0 or info[18] ==0:
        #     mawin.CtrlB.close()
        #     pass
    ...
    def TxBW_test(self):
        ...
        *****Start the whole test steps*****
        ...
        if not spec_TxRollOff[0]<np.max(np.max(judge_TxRolloff))<spec_TxRollOff[1]:
            print('TxBW failed detected: ')
            mawin.finalResult='Fail'
        if not spec_TxBW3DB[0]<np.min(np.min(judge_TxBW3DB))<spec_TxBW3DB[1]:
            print('TxBW 3dB failed detected: ')
            mawin.finalResult = 'Fail'
        if not spec_TxBW6DB[0]<np.min(np.min(judge_TxBW6DB))<spec_TxBW6DB[1]:
            print('TxBW 6dB failed detected: ')
            mawin.finalResult='Fail'
        print(mawin.finalResult)
        # Copy golden CSV data to folder under golden sample
        if mawin.test_type == '金样':
        ...
        #wb=app.books.open(mawin.report_model.replace('test_report.xlsx','test_report_TxBW.xlsx'))
        worksht=wb.sheets(1)
        worksht.activate()
        worksht.range((1,2)).value=test_result.iloc[0,0]
        worksht.range((2,2)).value=test_result.iloc[0,1]
        ...
        if not network_path == False:
            shutil.copytree(report_path3, network_path,dirs_exist_ok=True)
        wb = xw.Book(report_judge)
        #wb=app.books.open(report_judge)
        wb.sheets(1).activate()

    def TxBW_test_1123(self):
        '''
        #Tx BW test main process
        :return:
        '''
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            sw_ch = 2  # XI
            tt = []
            # add list to load 1.5G amplitude
            amp_list = []
            for j in range(4):
                if j == 0:
                    bias_ch = 1;
                    ...
                    sw_ch = 4  # YQ
                    self.sig_print.emit('start YQ channel test')
                zva.recall_cal(mawin, mawin.RFcal_path,
                               sw_ch)  # Load the calibration file according to the Date in the config file
                rfsw.switch_RFchannel(mawin, sw_ch)
                point = cb.get_quad(mawin, bias_ch, 500, 4000, 15)
                if point < 1000:
                    abc_ok[bias_ch - 1] = '0' + str(point)
                ...
                if '1' == zva.singleTest(mawin)[0]:
                    ...
                    if info[0][0] == '1':
                        ...
                        fre = df.iloc[:, 0]
                        ind = (abs(fre - fre_ref)).idxmin()
                        # here to get the raw amplitude
                        amp_list.append(raw.iloc[ind])
        ...
        if not spec_TxBW6DB[0] < np.min(np.min(judge_TxBW6DB)) < spec_TxBW6DB[1]:
            print('TxBW 6dB failed detected: ')
            mawin.finalResult = 'Fail'
        if not spec_amp[0] < np.min(np.min(amp_list)) < spec_amp[1]:
            print('Raw RF output 1.5G amplitude failed detected, XI,XQ,YI,YQ: \n', amp_list)
            mawin.finalResult = 'Fail'
        ...
        worksht.range((7, 2)).options(index=False, header=False, transpose=True).value = test_result.iloc[:, 5:]  # 26]
        if mawin.finalResult=='Fail':
            worksht.range((6, 2)).value='Fail'
        mawin.finalResult = worksht.range((6, 2)).value
        wb.save(report_judge)
        wb.close()
        # # record time and print out status,then close save log file
